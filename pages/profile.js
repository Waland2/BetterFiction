...
async function main() {
    try {
        ...
        const imagesParent = document.querySelectorAll('.mystories, .favstories');
        const images = document.querySelectorAll('[height="66"]');
        ...
        const bookmarkDataPromise = new Promise((resolve) => {
            chrome.runtime.sendMessage({ message: 'get-links' })
                ...
                .catch((error) => {
                    console.error('Failed to retrieve bookmark data for profile page:', error);
                    resolve([]);
                });
        });
            ...
            const addButtons = (place) => {
                ...
                newEl.addEventListener('click', () => {
                    ...
                    array.sort((a, b) => {
                        ...
                        try {
                            favCountA = Number(a.querySelector('.fav-cnt').innerText.replaceAll(',', ''));
                        } catch {
                            favCountA = 0;
                        }
                        try {
                            favCountB = Number(b.querySelector('.fav-cnt').innerText.replaceAll(',', ''));
                        } catch {
                            favCountB = 0;
                        }
