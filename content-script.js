...
async function main() {
    try {
        ...
        if (settings.moreOptionsInProfile) [['st', 'mystories', 0], ['fs', 'favstories', 1]].forEach(([place, storyType, sortType]) => {
        ...
        imagesParent.forEach(element => {
            if (settings.separateFics) Object.assign(element.style, {
            ...
            if (settings.bigCovers) {
                ...
                if (img) Object.assign(img.style, { width: '75px', height: '100px' });
            }
            ...
            if (descriptionDiv) {
                ...
                if (metaSpans[1]?.classList.contains('ratedmetatype')) metaSpans[0].innerHTML = metaSpans[0].innerHTML.replaceAll(placeholderText, ' - ');
        ...
        if (storyContrastButton) {
            ...
            if (settings.chapterWordCounter) {
                let wordCounter = 0;
                Array.from(document.querySelectorAll('p')).forEach(e => wordCounter += e.innerText.trim().split(/\s+/).length);
                chapSelects.forEach(e => e.options[e.selectedIndex].textContent += ` - ${wordCounter} words`);
            }
            ...
            if (settings.allFicButton && chapSelects[0]) {
                ...
                const sepSpan = (i, arr, className) => {
                    ...
                    if (settings.bookmarkButton && i !== arr.length) span.querySelector('h3').after(createBookmarkButton(i + 1, false));
                    return span;
                };
                ...
                allFicButton.addEventListener('click', async () => {
                    ...
                    chapSelects.forEach(e => e.parentElement.style.display = 'none');
                    const count = Number(document.querySelector('.chaptersmetatype-val').innerText);
                    const chaptersName = Array.from(chapSelects[0].options).map(o => o.textContent);
                        ...
                        onclick: async () => {
                            ...
                            for (let i = start; i < count; i++) {
                                try {
                                    ...
                                    const chapterElem = new DOMParser().parseFromString(await response.text(), 'text/html').querySelector('#storytext');
